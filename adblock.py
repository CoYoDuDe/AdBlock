...
class HybridStorage:
    def __init__(self, db_path: str):
        ...
        if os.path.exists(db_path):
            try:
                ...
                logger.warning(
                    f"Fehler beim Löschen der alten Datenbank {db_path}: {e}"
                )
    ...
    def calculate_threshold(self) -> int:
        try:
            ...
            threshold = max(
                20 * 1024 * 1024, min(512 * 1024 * 1024, int(free_memory * 0.1))
            )
            logger.debug(
                f"Schwellwert berechnet: {threshold/(1024*1024):.2f} MB (freier RAM: {free_memory/(1024*1024):.2f} MB)"
            )
            return threshold
        except Exception as e:
            logger.warning(
                f"Fehler bei Schwellwertberechnung: {e}, verwende Fallback: 20 MB"
            )
            return 20 * 1024 * 1024
    ...
    def update_threshold(self):
        ...
        if old_threshold != self.ram_threshold:
            logger.debug(
                f"Schwellwert aktualisiert: {self.ram_threshold/(1024*1024):.2f} MB"
            )
        self.use_ram = self.should_use_ram()
    ...
    def should_use_ram(self) -> bool:
        try:
            free_memory = psutil.virtual_memory().available
            logger.debug(
                f"Freier RAM: {free_memory/(1024*1024):.2f} MB, Schwellwert: {self.ram_threshold/(1024*1024):.2f} MB"
            )
            return (
                free_memory > self.ram_threshold
                or free_memory < 50 * 1024 * 1024
                or global_mode == SystemMode.EMERGENCY
            )
        except Exception as e:
            logger.warning(
                f"Fehler beim Überprüfen des freien RAM: {e}, verwende RAM-Speicher"
            )
            return True
    ...
    def reset_if_corrupt(self):
        try:
            if not hasattr(self, "db_path") or not self.db_path:
                logger.error(
                    "db_path nicht definiert, kann Datenbank nicht zurücksetzen"
                )
            ...
            if os.path.exists(self.db_path):
                ...
                logger.warning(
                    f"Datenbank {self.db_path} zurückgesetzt wegen Korruption"
                )
    ...
    def __setitem__(self, key: str, value: Any):
        try:
            ...
            logger.error(
                f"Kritischer Fehler beim Schreiben in HybridStorage: key={key}, value_type={type(value)}, error={e}"
            )
            raise
    ...
    def __init__(self, url: str):
        ...
        try:
            ...
            if not isinstance(root_node, TrieNode):
                logger.warning(
                    f"Root-Knoten ungültig (Typ: {type(root_node)}), initialisiere neu"
                )
    ...
    def insert(self, domain: str):
        try:
            ...
            try:
                ...
                if not isinstance(root_node, TrieNode):
                    logger.error(
                        f"Root-Knoten ist kein TrieNode, sondern: {type(root_node)}"
                    )
                    self.storage.reset_if_corrupt()
                    self.storage[self.root_key] = TrieNode()
            except KeyError:
...
def load_trie_cache(all_domains_hash: str, url: str) -> Optional[DomainTrie]:
    if (
        not CONFIG["cache_trie"]
        or not os.path.exists(TRIE_CACHE_PATH)
        or global_mode == SystemMode.EMERGENCY
    ):
...
class CacheManager:
    def __init__(self, db_path: str, flush_interval: int):
        ...
        self.init_database()
        logger.info(
            f"CacheManager initialisiert: Initiale Cache-Größe={self.current_cache_size}"
        )
    ...
    def calculate_dynamic_cache_size(self) -> int:
        try:
            ...
            if (
                free_memory
                < CONFIG["resource_thresholds"]["low_memory_mb"] * 1024 * 1024
            ):
                dynamic_size = 2
                log_once(
                    logging.WARNING,
                    "Low-Memory-Modus aktiviert: Cache-Größe auf 2 reduziert",
                    console=True,
                )
            logger.debug(
                f"Dynamische Cache-Größe berechnet: {dynamic_size} (freier RAM: {free_memory/(1024*1024):.2f} MB)"
            )
            return dynamic_size
        except Exception as e:
            logger.warning(
                f"Fehler bei Cache-Größenberechnung: {e}, verwende Fallback: 2"
            )
            return 2
        ...
        try:
            ...
            if new_size != self.current_cache_size:
                ...
                if (
                    ...
                    logger.debug(
                        f"Domain-Cache beschnitten auf {self.current_cache_size} Einträge"
                    )
        except Exception as e:
        ...
        while True:
            try:
                ...
                if cpu_usage > 90:
                    log_once(
                        logging.WARNING,
                        f"Hohe CPU-Auslastung: {cpu_usage}%, reduziere parallele Aufgaben",
                        console=True,
                    )
                logger.debug(
    ...
    def save_domain_cache(self):
        try:
            ...
            logger.debug(
                f"Domain-Cache geflusht (Flush #{STATISTICS['cache_flushes']})"
            )
        except Exception as e:
...
def cleanup_temp_files(cache_manager: CacheManager):
    try:
        ...
        for file in os.listdir(TMP_DIR):
            if (
                file.endswith(".tmp")
                or file.endswith(".filtered")
                or file.startswith("trie_cache_")
            ):
                    ...
                    if url in list_cache:
                        ...
                        if last_checked < expiry:
                            try:
                                os.remove(file_path)
                                logger.info(
                                    f"Veraltete Trie-Cache-Datei gelöscht: {file}"
                                )
                            except Exception as e:
...
async def is_ipv6_supported() -> bool:
    try:
        ...
        logger.debug(
            "IPv6 wird unterstützt (erfolgreicher AAAA-Lookup über IPv6-DNS-Server)"
        )
        return True
...
async def monitor_resources(cache_manager: CacheManager):
    ...
    consecutive_violations = {"low_memory": 0, "high_cpu": 0, "high_latency": 0}
    history = {
    ...
    def print_system_status():
        log_once(
            ...
            f"- Angepasste Batch-Größe: {batch_size} Domains",
            console=True,
        )
    ...
    while True:
        try:
            ...
            if avg_memory < CONFIG["resource_thresholds"]["emergency_memory_mb"]:
                ...
                avg_memory < CONFIG["resource_thresholds"]["low_memory_mb"]
                or avg_cpu > CONFIG["resource_thresholds"]["high_cpu_percent"]
                or avg_latency > CONFIG["resource_thresholds"]["high_latency_s"]
            ):
            ...
            if new_mode != resource_state["mode"]:
                log_once(
                    logging.INFO,
                    f"Betriebsmodus gewechselt: {resource_state['mode'].value} → {new_mode.value}",
                    console=True,
                )
            ...
            if avg_memory < CONFIG["resource_thresholds"]["low_memory_mb"]:
                ...
                if (
                    consecutive_violations["low_memory"]
                    >= CONFIG["resource_thresholds"]["consecutive_violations"]
                    and not resource_state["low_memory"]
                ):
                    log_once(
                        logging.WARNING,
                ...
                if resource_state["low_memory"]:
                    log_once(
                        logging.INFO,
                        f"RAM wieder ausreichend verfügbar: Durchschnitt {avg_memory:.2f} MB frei",
                        console=True,
                    )
            ...
            if avg_cpu > CONFIG["resource_thresholds"]["high_cpu_percent"]:
                ...
                if (
                    consecutive_violations["high_cpu"]
                    >= CONFIG["resource_thresholds"]["consecutive_violations"]
                    and not resource_state["high_cpu"]
                ):
                    log_once(
                        logging.WARNING,
                        f"Hohe CPU-Auslastung erkannt: Durchschnitt {avg_cpu:.1f}%, reduziere parallele Jobs",
                        console=True,
                    )
                ...
                if resource_state["high_cpu"]:
                    log_once(
                        logging.INFO,
                        f"CPU-Auslastung wieder normal: Durchschnitt {avg_cpu:.1f}%",
                        console=True,
                    )
            ...
            if avg_latency > CONFIG["resource_thresholds"]["high_latency_s"]:
                ...
                if (
                    consecutive_violations["high_latency"]
                    >= CONFIG["resource_thresholds"]["consecutive_violations"]
                    and not resource_state["high_latency"]
                ):
                    log_once(
                        logging.WARNING,
                        f"Hohe Netzwerklatenz erkannt: Durchschnitt {avg_latency:.2f}s, DNS-Anfragen reduziert",
                        console=True,
                    )
                ...
                if resource_state["high_latency"]:
                    log_once(
                        logging.INFO,
                        f"Netzwerklatenz wieder normal: Durchschnitt {avg_latency:.2f}s",
                        console=True,
                    )
...
def get_system_resources() -> tuple[int, int, int]:
    try:
        ...
        if global_mode == SystemMode.EMERGENCY:
            ...
            log_once(
                logging.WARNING,
                f"Low-Memory-Modus: Batch-Größe={batch_size}, Jobs={max_jobs}, DNS-Anfragen={max_concurrent_dns}",
                console=True,
            )
        else:
            max_jobs = max(
...
async def test_dns_entry_async(
    ...
    async def query_with_backoff(
        ...
        try:
            ...
            logger.debug(
                f"Unbekannter Fehler bei {domain} ({record}, Versuch {attempt + 1}): {e}"
            )
            return False
    ...
    try:
        ...
        async with semaphore:
            record_types = (
                [record_type, "AAAA"] if record_type == "A" else [record_type]
            )
            for record in record_types:
                for attempt in range(CONFIG["max_retries"]):
                    reachable = await query_with_backoff(
                        domain, record, resolver, attempt
                    )
                    if reachable:
                        with dns_cache_lock:
                    ...
                    delay = (
                        ...
                        if CONFIG["dns_retry_strategy"] == "exponential"
...
def setup_git() -> bool:
    try:
        ...
        if not os.path.exists(git_dir):
            logger.error(
                "Git-Repository nicht gefunden. Bitte initialisieren Sie das Repository manuell."
            )
            return False

        # Prüfe, ob das Remote-Repository korrekt konfiguriert ist
        result = subprocess.run(
            ["git", "remote", "-v"], capture_output=True, text=True, cwd=SCRIPT_DIR
        )
        if CONFIG["github_repo"] not in result.stdout:
            logger.debug(f"Füge Remote-Repository hinzu: {CONFIG['github_repo']}")
            subprocess.run(
                ["git", "remote", "add", "origin", CONFIG["github_repo"]],
                check=True,
                cwd=SCRIPT_DIR,
            )
        else:
        ...
        branch_result = subprocess.run(
            ["git", "branch", "--list", CONFIG["github_branch"]],
            capture_output=True,
            text=True,
            cwd=SCRIPT_DIR,
        )
        if CONFIG["github_branch"] not in branch_result.stdout:
            ...
            subprocess.run(
                ["git", "checkout", "-b", CONFIG["github_branch"]],
                check=True,
                cwd=SCRIPT_DIR,
            )
        else:
            ...
            subprocess.run(
                ["git", "checkout", CONFIG["github_branch"]], check=True, cwd=SCRIPT_DIR
            )
...
def upload_to_github():
    ...
    try:
        ...
        if new_hash == old_hash:
            ...
            return
        result = subprocess.run(
            ["git", "status", "--porcelain", CONFIG["hosts_file"]],
            capture_output=True,
            text=True,
            cwd=SCRIPT_DIR,
        )
        ...
        send_email(
            "Warnung im AdBlock-Skript",
            f"Git-Upload fehlgeschlagen: {e}\nStderr: {e.stderr}",
        )
    except Exception as e:
